...
<Image src="https://files.readme.io/25d9fc2-Screenshot_203.png" />
...
year - returns the year from the date e.g. currentCustomer.extField_dob_date.year( )>=1981 and ¤tCustomer.extField_dob_date.year( )<=1996\
...
You can chain together multiple valid expressions using AND ( and  and ) or an OR (||) condition between them
...
e.g. to check if a customer has either bought for $100 or more in a completely non-discounted transaction, or for $300 or more then you can say ((currentTxn.value>=100) and  and (currentTxn.basketSumIf("DISCOUNT","NOT_EQUAL_TO","0")==0))||(currentTxn.value>=300)
...
currentTxn.value>1000 and  and (currentTxn.value<5000)
...
(currentTxn.value>1000 and  and (currentTxn.value<5000) and  and (currentCustomer.name.matches("John Smith"))
...
currentTxn.value==5000 and  and (currentCustomer.firstname.matches("John"))
...
currentCustomer.numberOfVisits==2 and  and (currentCustomer.avgSpendPerVisit==4750)
